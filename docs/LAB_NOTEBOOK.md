...
| 2026-07-30 | **NEW FLAGSHIP E13: ground-truth validation benchmark for activation-verbalization methods.** The field DEPLOYS verbalizers (NLA, Patchscopes, SelfIE, LatentQA) in production audits with no way to check them. Plant content, measure 3 axes: (1) **recovery** vs oracle; (2) **attribution** — right reason or steering/inference (our neutral-framing control; operationalises the "excessive expressivity" limitation they name but can't test); (3) **CONFABULATION RATE** — inject NOTHING, ask what's there, count false assertions. That FPR is a number the field needs, cannot produce, and we can generate at 2B. Extends arXiv:2509.13316 (compares Patchscopes/SelfIE/LatentQA/probes) by adding planted ground truth + the steering control. **Compute inverts in our favour**: Patchscopes/SelfIE/logit-lens are INFERENCE-ONLY, free-tier runnable now; NLA needs RL-training 2 LLMs → out of reach, scope it honestly | Addendum 9, §A9.3 |
| 2026-07-30 | **Adopt:** meta-d'/M-ratio for L4 (mature; note arXiv:2603.25112 already applies SDT to LLMs → adoption not novelty); seed-twin control for privileged-access claims; disagreement-subset stratification. **DROP: the developmental/training-checkpoint arm (U2, B9)** — partially claimed (arXiv:2606.02378, 2509.05291; introspection reported emerging at DPO stage on OLMo-3.1-32B), expensive, competes with E13 for the Sem-6 window. **RISK on E13: Anthropic INVENTED concept injection** (Lindsey) and could combine it with NLA to close their own gap at any time → hedge by moving fast (E13 is inference-only, Sem 5), making the BENCHMARK the durable asset (adoption beats novelty), and owning the confabulation-rate metric specifically | Addendum 9, §A9.4-6 |
| 2026-07-30 | **REPOSITIONED — the field is crowding; sell shovels not gold.** SPAR F26 list read as a field snapshot: ~15 of 211 projects are directly on introspection/self-report/verbalization in ONE cohort (Anthropic training verbalizations to match activations; DeepMind on trusting self-explanations; LASR planting cues to measure the verbalization gap = OUR method; "Red-teaming Model Self-reports" = OUR R11/R12), plus ~5 persona, ~4 eval-awareness, ~6 CoT-faithfulness. Our preprint is Apr 2027, submission 2028 → the empirical question will be crowded, not open. **KEY ASYMMETRY: empirical assets DEPRECIATE with crowding, methodological assets APPRECIATE** — every new introspection paper needs ground truth, a decodability baseline, a steering control, and a null, all of which we have built. **NEW THESIS: "self-knowledge claims are systematically confounded by output steering; here is the confound, the control protocol, and a demonstration it overturns a real result (including our own)"** — the Gemma null becomes the demonstration case, not the headline | Addendum 8, §A8.1-3 |
| 2026-07-30 | **Five updates from the repositioning:** (1) flip the paper's centre of gravity; (2) **NEW AUDIT ARM** — test which published introspection claims survive a neutral-framing control (cheap, no GPU, fits Sem 5, and is only POSSIBLE because the field is crowded → crowding becomes an input not a threat); (3) adopt **NLA as a second readout** → 3-way PRG (probe/NLA/verbal report); (4) adopt the **seed-twin control** (identical arch/tokenizer/data, different init) for any privileged-access claim incl. our own — built while critiquing arXiv:2511.08579, whose self-vs-other comparison conflates access with representational compatibility; (5) **PLANTED promoted to first-class deliverable** | Addendum 8, §A8.4 |
| 2026-07-30 | **AMBITION RECALIBRATED (honest).** "Most jaw-dropping paper of all time" restated: an undergrad capstone on free-tier compute in a subfield worked by Anthropic/DeepMind will not produce a landmark empirical result, and chasing one biases toward breadth/novelty when the achievable edge is rigour and reusability. **New target: the methods paper + benchmark this subfield uses for 5 years.** Achievable with the compute we actually have; the old target was not | Addendum 8, §A8.5 |
| 2026-07-25 | **Benchmarked against departmental output + RUBRIC-MISMATCH RISK FOUND.** PES CS ships 194-210 capstones/yr (53-75 shortlisted); standard route is IEEE/Scopus proceedings; panels include industry judges (VISA, Reliance Jio); no PES undergrad paper found at NeurIPS/ICML/ICLR/ACL and no interpretability group at PES. Our position: **methodology far outside the local distribution** (pre-reg + falsification reported, 3 control conditions, causal patching, CIs, explicit cannot-claim table); **venue tier several steps up, not one** (~20-27% vs 40-60%+); **completeness currently BEHIND a typical Sem-7 capstone** (they demo a system, we have a pilot + compute dependency). RISK: the rubric ("module implementation", "deployment", "demonstration") + industry judges evaluate SYSTEMS, but we deliver a negative FINDING — "the model failed to notice" can read as "the project failed". Mitigations: lead with the apparatus not the null; demo = rubric insurance; safety framing first; assume mentor is a non-specialist | Addendum 7, §A7.10 |
